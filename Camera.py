...
class SimpleCamera:
	def __init__(self):
		...
		self.reviousCursor = glm.vec2(0.0)
	...
	def MouseMove(self, X: float, Y: float):
		if self.bEnableMouseMovement:
			...
			if (Delta.sizeof() < 5.0):
				self.Right = glm.vec3(glm.cross(self.Direction, self.Up))

				RotationRight = glm.rotate(glm.identity < glm.mat4 > (), glm.radians(-Delta.y), self.Right)
	...
	def Update(self, DeltaTime: float):
		glm.vec3(glm.cross(self.Direction, self.Up))
		self.Location += self.Direction * self.ForwardScale * DeltaTime
		self.Location += self.Right * self.RightScale * DeltaTime
